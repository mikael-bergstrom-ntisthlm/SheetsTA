...
export namespace LibGClassroom {
   ...
   * @returns {string[][]} A two-dimensional array; an array of rows containing assignment data.
   *   First row is headers. Each row (inner array) will contain) columns: Title, CourseID, CourseworkID
   */
  export function GetAssignments(config: LibConfig.Config): string[][] {
   ...
   * @returns {string[][]} A two-dimensional array; an array of rows containing submission attachment data. 
   *   First row is headers. Each row (inner array) will contain) columns: UserID, CourseID, CourseworkID (assignment ID), 
   *   State (turned in, created etc), MIME, Submission URL.
   */
  export function GetStudentSubmissions(config: LibConfig.Config): string[][] {
        ...
        submissions.studentSubmissions.forEach(submission => {
          ...
          submission.assignmentSubmission?.attachments.forEach(attachment => {
            ...
            submissionValues.push([
              ...
              attachmentType,
              attachmentMimeType,
              LibGithub.UrlSanitize(attachmentUrl)
            ])
          });
        });
   ...
   * @param {GoogleAppsScript.Classroom.Schema.Attachment} attachment - The attachment to analyze
